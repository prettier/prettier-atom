...
const warnAboutLinterEslintFixOnSave = require('./warnAboutLinterEslintFixOnSave'); // eslint-disable-next-line global-require


const displayDebugInfo = require('./displayDebugInfo'); // eslint-disable-next-line global-require,prefer-destructuring


const toggleFormatOnSave = require('./atomInterface').toggleFormatOnSave; // local helpers
...
const attachStatusTile = () => {
  if (statusBarHandler) {
    ...
    subscriptions.add( // onDidChangeActiveTextEditor is only available in Atom 1.18.0+.
...
const loadPackageDeps = () => // eslint-disable-next-line global-require
require('atom-package-deps').install('prettier-atom') // eslint-disable-next-line no-console
.then(() => console.log('All dependencies installed, good to go')); // public API
...
const consumeIndie = registerIndie => {
  ...
  subscriptions.add(linter); // Setting and clearing messages per filePath
