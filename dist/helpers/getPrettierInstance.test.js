...
test("returns user's project's local prettier instance if it exists", () => {
  ...
  expect(atomLinter.findCached).toHaveBeenCalledWith(path.join(__dirname, '..', '..', 'tests', 'fixtures'), path.join('node_modules', 'prettier', 'index.js'));
});
test("returns global prettier (by npm) if user's project has no local prettier package", () => {
  ...
  atomLinter.findCached.mockImplementation(dir => dir === globalModules ? fakeGloballyInstalledPrettier : null);
  const actual = getPrettierInstance(editor);
  ...
  expect(atomLinter.findCached).toHaveBeenLastCalledWith(globalModules, path.join('node_modules', 'prettier', 'index.js'));
});
test("returns global prettier (by yarn) if user's project has no local prettier package", () => {
  ...
  const fakeGloballyInstalledPrettier = path.join(__dirname, '..', '..', 'tests', 'fixtures', 'prettier.js');
  atomLinter.findCached.mockImplementation(dir => dir === yarnGlobalModules ? fakeGloballyInstalledPrettier : null);
  const actual = getPrettierInstance(editor);
  expect(actual).toEqual(require(fakeGloballyInstalledPrettier)); // eslint-disable-line

  expect(atomLinter.findCached).toHaveBeenCalledTimes(3);
  expect(atomLinter.findCached).toHaveBeenLastCalledWith(yarnGlobalModules, path.join('node_modules', 'prettier', 'index.js'));
});
