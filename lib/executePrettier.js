...
const useAtomTabLengthIfAuto = tabLength =>
  tabLength === 'auto' ? atom.config.get('tabLength') : tabLength;

const getPrettierOptions = () => ({
  printWidth: getConfigOption('printWidth'),
  tabWidth: useAtomTabLengthIfAuto(getConfigOption('tabWidth')),
...
const executePrettier = (text) => {
  try {
    return prettier.format(text, getPrettierOptions());
  } catch (error) {
...
const executePrettierOnBufferRange = (editor, bufferRange) => {
  ...
  const transformed = executePrettier(textToTransform);
